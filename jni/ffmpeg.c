...
{
	ffmpeg_cleanup(ret);
	// Need to reset all static variables for the next run
...
void exit_program_at_the_end(int ret)
{
	ffmpeg_cleanup(ret);
	// Need to reset all static variables for the next run
	frame_bits_per_raw_sample = 0;
                         ...
                         double sync_ipts)
{
    ...
    if (delta0 < 0 &&
        ...
        if (delta0 < -0.6) {
            ...
            LOGD( "Cliping frame in rate conversion by %f\n", -delta0);
        sync_ipts += cor;
        duration -= cor;
        delta0 += cor;
    }
...
//                    ost->frame_rate = ist->st->avg_frame_rate.num ? ist->st->avg_frame_rate : (AVRational){25, 1};
    ...
    for (i = 0; i < nb_output_streams; i++) {
        ...
        if (ost->encoding_needed) {
            ...
            LOGW( "opening codec %s", codec->name);
...
int main(int argc, char **argv)
{
   ...
   // register_exit(ffmpeg_cleanup);
    ...
    if (nb_output_files <= 0) {
        LOGE( "At least one output file must be specified %d \n", nb_output_files);
        exit_program(1);
    }
...
//     }
    ...
    exit_program_at_the_end(received_nb_signals ? 255 : main_return_code);
